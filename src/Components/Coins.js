                        ...
                        <div className='col-sm-12'>
                            <div className='card shadow'>
                                <div className='card-header'>
                                    <div className='title'>
                                        <FontAwesomeIcon icon={faCoins}></FontAwesomeIcon>  eNaira Wallet
                                    </div>
                                </div>
                                <div className='card-body'>
                                    ...
                                    <div className='row'>
                                            ...
                                            <ul className=' list-group'>
                                                ...
                                                <li className='list-group-item'>
                                                    <button className='btn text-danger' onClick={() => {
                                                        ...
                                                        <FontAwesomeIcon icon={faMoneyBill}></FontAwesomeIcon> Fund Wallet
                                                    </button>
                                                    <button className='btn text-microskool' onClick={() => {
                                                        dispatch(setTransferCoins(true))
                                                    }}>
                                                        <FontAwesomeIcon icon={faShare}></FontAwesomeIcon> Transfer Coins
                                                    </button>
                                                    <button className='btn text-warning' onClick={() => {
                                                        dispatch(setWithdrawCoins(true))
                                                    }}>
                                                        <FontAwesomeIcon icon={faBank}></FontAwesomeIcon> Widthdraw
                                                    </button>
                                                </li>
                                            </ul>
                                        </div>
                                        ...
                                        <div className='col-sm-9'>
                                            {addCourse ? <Modal config={{ align: 'flex-end', justify: 'right' }} header={'Create a New Course'} body={<>
                                                ...
                                                createCourse({ code, title, department, level })
                                            }}>Add</button><button className='btn-close' onClick={() => dispatch(setaddCourse(false))}></button></>} /> : <></>}
                                                    ...
                                                    <tbody>
                                                        {
                                                            transactions.map((trans) => {
                                                                return <tr>
                                                                    ...
                                                                    <td>
                                                                        {trans.receiver === user.email ? trans.description_receiver : trans.description_sender}
                                                                    </td>
                                                                    <td>
                                                                        {trans.sender}
                                                                    </td>
                                    ...
                                    {fundCoins ? <Modal config={{ align: 'flex-start', justify: 'left' }} header={'Fund your Wallet'} body={<>
                                        <input placeholder='Amount' className='form-control' type={'number'} onChange={(e) => {
                                            setAmount(e.target.value)
                                            setcharge((0.9 * e.target.value) / 100)


                                        }} />
                                        <span className='badge bg-secondary'>Transaction charge (0.9%): <FontAwesomeIcon icon={faCoins}></FontAwesomeIcon> {charge}</span>
                                    </>} footer={<>
                                        <PaystackButton className="btn microskool-button" {...componentProps} /><button className='btn-close' onClick={() => dispatch(setFundCoins(false))}></button></>} /> : <></>}



                                    {withdrawCoins ? <Modal config={{ align: 'flex-start', justify: 'left' }} header={'Withdraw to eNaira'} body={<>
                                        <input placeholder='Amount' className='form-control' type={'number'} onChange={(e) => {
                                            setAmount(e.target.value)
                                            setcharge((0.9 * e.target.value) / 100)
                                        ...
                                        <input placeholder='eNaira ID ' className='form-control' type={'text'} onChange={(e) => {
                                       ...
                                       <button className='btn microskool-button' onClick={()=>{
                                            axios.post('http://192.168.43.31:5000/transactions/validate', { sender: user.email, amount, password: user.password }).then((resp) => {

                                                if (resp.data.success) {
                                                    axios.post('http://192.168.43.31:5000/transactions', { transaction_id: 'T255' + Math.random() * 2344354, item: 'Coins Withdrawal', description_sender: 'Coins withdrawal request made by you', description_receiver: eNaira, sender: user.email, receiver: 'Microskool', amount, status: 'Pending' }).then((res) => {
if(res.data.success){
    dispatch(setalert({ ...alert, msg: 'Withdrawal requested successfully, your request would be processed within 24 hours', type: 'success', status: true, cap: 'Success' }))
    ...
    dispatch(setalert({ ...alert, msg: res.data.message, type: 'danger', status: true, cap: 'Error' }))
                                                    ...
                                                    dispatch(setalert({ ...alert, msg: resp.data.message, type: 'danger', status: true, cap: 'Error' }))

                                                }

                                       })
                                       }}>Withdraw <FontAwesomeIcon icon={faCoins}></FontAwesomeIcon>{amount} Now </button> <button className='btn-close' onClick={() => dispatch(setWithdrawCoins(false))}></button></>} /> : <></>}




                                    {transferCoins ? <Modal config={{ align: 'flex-start', justify: 'left' }} header={'Transfer'} body={<>
                                        <input placeholder='Email' className='form-control' type={'text'} onChange={(e) => {
                                        ...
                                        {
                                            processed ? <div>
                                                <img src={ben.image} alt='image' style={{ width: '100px', borderRadius:'12px' }} />
                                                <h5>{ben.first_name} {ben.surname} </h5>
                                                {ben.institution}
                                        ...
                                        {
                                            processed ? <button className="btn microskool-button" onClick={()=>{
                                                axios.post('http://192.168.43.31:5000/transactions/validate', { sender: user.email,  amount, password: pass }).then((res) => {
...
if(res.data.success){
                                                ...
                                                axios.post('http://192.168.43.31:5000/transactions', { transaction_id: 'T255'+Math.random()*2344354, item: 'Coins Transfer', description_sender: 'Coins Transfer made by you to '+ben.first_name+' '+ben.surname, description_receiver: 'Coins transferred to you from '+user.first_name+' '+user.surname, sender: user.email, receiver: ben.email, amount, status:'Approved' }).then((res) => {
                                                if(res.data.success){
                                                    axios.post('http://192.168.43.31:5000/users/' + user.email + '', { ...user, coins: parseFloat(user.coins) - parseFloat(amount) }).then((response) => {
                                                        dispatch(setalert({ ...alert, msg: response.data.message, type: 'info', status: true, cap: 'Info' }))

                                                })
                                                    axios.post('http://192.168.43.31:5000/users/' + ben.email + '', { ...ben, coins: parseFloat(ben.coins) + parseFloat(amount) }).then((response) => {
                                                        dispatch(setalert({ ...alert, msg: response.data.message, type: 'info', status: true, cap: 'Info' }))
                                                    ...
                                                    dispatch(setalert({ ...alert, msg: 'Transaction Failed', type: 'danger', status: true, cap: 'Error' }))   
                                                }
                                            })
                                        }else{
    dispatch(setalert({ ...alert, msg: res.data.message, type: 'danger', status: true, cap: 'Error' }))
                                            ...
                                            }}>Send {amount}</button> : <button className="btn microskool-button" onClick={process} >Process</button>
                                        }
                                        <button className='btn-close' onClick={() => dispatch(setTransferCoins(false))}></button></>} /> : <></>}
