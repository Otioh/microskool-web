...
} else{ axios.get('http://192.168.43.31:5000/users/'+id+'').then((res)=>{
  if(res.data.success){ 
...
{ withdrawCoins?<Modal config={{align:'flex-start', justify:'left'}} header={'Withdraw'} body={<>
    <input placeholder='Amount' className='form-control' type={'number'} onChange={(e)=>{
      setAmount(e.target.value)
  setcharge((0.9*e.target.value)/100)
...
{
  processed?<div>
<img src={ben.image} alt='image' style={{width:'80%'}} />
...
:<></>
      ...
      {
        processed?<button className="btn microskool-button" >Send {amount}</button>:<button className="btn microskool-button" onClick={process} >Process</button>
      }
 <button className='btn-close' onClick={()=>dispatch(setFundCoins(false))}></button></>} />:<></>}
